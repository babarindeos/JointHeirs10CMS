        ...
        <section class="flex flex-col w-[95%] md:w-[95%] py-1 mt-6 px-4 border-red-900 mx-auto">
            ...
            <div class="flex border-b border-gray-300 py-2 justify-between">
                    <div class="flex flex-row w-1/4">
                        <h1 class="text-2xl font-semibold font-serif text-gray-800">Members</h1>
                    </div>
                    <div class="flex flex-row w-3/4 justify-end space-x-1">

                            
                            <div class="border-0 flex flex-col items-center justify-center">
                                        <a href="{{ route('admin.members.create') }}" class="bg-white-600 border border-green-600 text-green-800 py-2 px-4 
                                                rounded-lg text-sm hover:bg-green-500 hover:text-white">New Member</a>
                            </div>
                    </div>
            </div>
        </section>
               ...
               <section class="flex flex-col py-2 px-2 justify-end w-[93%] mx-auto md:px-1">
                    <form action="{{ route('admin.members.index') }}" method="GET">
                            <div class="flex justify-end border border-0">
                                ...
                                /> <button class='text-sm px-2 border rounded-r-md bg-green-600 text-white'>Search</button> 
        
                            </div>
                    </form>
                    
                </section>
                <div class='py-2 w-[93%] md:w-[93%] mx-auto'>{{ $members->links() }}</div>                    

                <section class="flex flex-wrap gap-6 mx-auto w-[93%] md:w-[93%] md:flex-row justify-center space-y-2 md:space-y-0  border-0 py-2 mb-16">
                                        ...
                                        <div class='flex flex-col w-full md:flex-row md:w-[27%] border rounded-xl shadow-md px-5 py-6 bg-white'>
                                                <div class='flex flex-col w-full items-center justify-center'>
                                                    ...
                                                    <div class='flex flex-col text-lg font-semibold py-1 justify-center text-center items-center'>
                                                                        <a class='hover:overline underline' href="{{ route('admin.members.show', ['uuid' => $member->uuid]) }}">
                                                    ...
                                                    <div class="text-center py-2">
                                                            <span class="px-1">
                                                                <a class="bg-green-400 hover:bg-green-500 text-white rounded-md px-4 py-1 text-xs"
                                                                    href="{{ route('admin.members.show', ['uuid'=>$member->uuid]) }}">
                                                                    View
                                                                </a>
                                                            </span>
                                                            <span class="text-sm">
                                                                <a class="hover:bg-blue-500 bg-blue-400 text-white rounded-md 
                                                                        px-4 py-1 text-xs" href="{{ route('admin.members.edit', ['uuid' => $member->uuid]) }}">Edit</a>
                                                            </span>
                                                            <span> 
